...
int cache_miss(int cache_sets, int associativity, int reference_list_length, int tag_bits, int indexing_bit_count, int index_bit_position) 
{
    ...
    for(int i = 1; i < reference_list_length - 1; i++){
        ...
        int bit_position = 0; // the Most Significant Bit

        // using tag_bits, reference_list to extract tag
        for(bit_position; bit_position < tag_bits+indexing_bit_count; bit_position++){
            // std::cout << "bit_position = " << bit_position << endl;
            if(bit_position == index_bit_position+1-indexing_bit_count){
                // std::cout << "indexing_bit_count = " << indexing_bit_count << endl;
                bit_position += indexing_bit_count-1;
            }
            else{
        ...
        for(int k = index_bit_position+1-indexing_bit_count; k <= index_bit_position; k++){
            index += reference_list[i][k];
        }
...
int main(int argc, char* argv[]) 
{
    ...
    // choose the random bits of block address (skipping offset bits) to decide the cache set.
    int min = offset_bits_count;
    int max = address_bits - indexing_bit_count;
    srand( time(NULL) );

    double runtime = 0;
    int best_total_miss_count = 999999;
    int best_index_position;
    while (runtime < 25){
        double START,END;
        START = clock();
        int index_bit_position = rand() % (max - min + 1) + min;
        // std::cout << "index_bit_position: " << index_bit_position << endl;
        for(int i = index_bit_position, j = 0; j < indexing_bit_count; i++, j++){ 
            indexing_bits[j] = i;
        }
        // cache behavior simulation
        int total_miss_count = cache_miss(cache_sets, associativity, reference_list_length, tag_bits, indexing_bit_count, (address_bits - 1 - index_bit_position));
        if (total_miss_count < best_total_miss_count){
            best_total_miss_count = total_miss_count;
            best_index_position = address_bits - 1 - index_bit_position;
        }
        END = clock();
        runtime += (END - START) / CLOCKS_PER_SEC;
    }
    // make a best index reference list (hit/miss)
    cache_miss(cache_sets, associativity, reference_list_length, tag_bits, indexing_bit_count, best_index_position);
    ...
    file_out << endl;
    file_out << "Offset bit count: " << offset_bits_count << endl;
    ...
    file_out << "Total cache miss count: " << best_total_miss_count << endl;
    file_out.close();
